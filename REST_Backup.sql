...
-- Schema: BANDFINDER   Date: Sun Jan 24 14:46:19 CET 2021
--
BEGIN
  ORDS.ENABLE_SCHEMA(
      p_enabled             => TRUE,
...
'SELECT
    ...
    (SELECT LISTAGG(instr.name, '', '') 
     ...
     (SELECT MAX(play_str.description) 
      ...
      (SELECT TO_CHAR(MAX(usr_instr.started_playing), ''YYYY-MM-DD'')
       FROM user_instruments usr_instr
       WHERE usr.usr_id = usr_instr.usr_id) as started_playing,
       med.title,
       med.content_type,
...
WHERE usr.usr_id = :usr_id
   OR (:usr_id IS NULL AND usr.session_key = :session_key AND usr.session_expire_date > sysdate)
...
EXCEPTION
    WHEN others THEN
    :status := 400;
    :err_msg := ''Error updateing user: '' || SQLERRM();
END;'
      );
  ORDS.DEFINE_PARAMETER(
      p_module_name        => 'BANDFINDER',
